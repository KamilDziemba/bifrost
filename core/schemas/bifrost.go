...
const (
	...
	BifrostContextKeyOAuth2JWTAuthenticated              BifrostContextKey = "bifrost-oauth2-jwt-authenticated"                 // bool (true when the /mcp request was authenticated via a Bifrost-issued JWT rather than a raw header credential; set by the /mcp JWT verification path)
	BifrostContextKeyOAuth2JWTSessionValidated           BifrostContextKey = "bifrost-oauth2-jwt-session-validated"             // bool (true when JWTUserValidator confirmed the user's session is still active; absent when no validator is registered or mode is not user)
